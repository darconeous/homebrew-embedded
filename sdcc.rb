...
class Sdcc < Formula
  ...
  sha256 'd9dc4b8d4fa25386ad065133b055d8e62714944c6db07b063ad544a38252a08f'
  ...
  def install
    ...
    if build.include? 'enable-avr-port'
      ohai 'AVR Port enabled.'
      args << '--enable-avr-port'
    end

    if build.include? 'enable-xa51-port'
      ohai 'XA51 Port enabled.'
      args << '--enable-xa51-port'
    end
  ...
  def patches
    ...
    patch_list << 'https://gist.github.com/anonymous/5042275/raw/a2e084f29cd4ad9cd95e38683209991b7ac038d3/sdcc-huge-stack-auto.diff'

    # The issue the patch below fixes is already fixed on HEAD, so
    # we only want to apply it if we aren't building for HEAD.
    patch_list << DATA if !build.head?
  end
